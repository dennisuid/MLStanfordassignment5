...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### START CODE HERE
        # string = "John Fisher. Born in Glasgow, Fisher is a graduate of the London Opera Centre, the Royal Academy of Music, and the University of Glasgow"
        # inp1, oup1 = string.split('\t')
        # inp, oup = self.data[idx].split('\t')
        # x = inp + self.MASK_CHAR + oup + self.MASK_CHAR
        # x = x + self.PAD_CHAR * (self.block_size - len(x))
        # y = self.PAD_CHAR * (len(inp) - 1) + x[len(inp):]
        document = str(self.data[idx])
        document_len = len(document)
        if document_len > int(self.block_size*7/8):
            truncate_len = random.randint(4, int(self.block_size*7/8))
        else:
            truncate_len = random.randint(4, document_len)
        truncated_doc = document[:truncate_len]
        masked_content_len = int(random.triangular(0, truncate_len, truncate_len/4))
        prefix_len = random.randint(0, int((truncate_len - masked_content_len)/2))
        prefix = truncated_doc[:prefix_len]
        masked_content = truncated_doc[prefix_len:prefix_len + masked_content_len]
        suffix = truncated_doc[prefix_len + masked_content_len:]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR + self.PAD_CHAR * (self.block_size - (len(document)+3))
        x = masked_string[:-1]
        y = masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
